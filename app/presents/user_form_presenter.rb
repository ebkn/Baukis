class UserFormPresenter < FormPresenter
  def password_field_block(name, label_text, options = {})
    ...
    markup(:div, class: 'form-group py-2') do |m|
      ...
      m << password_field(name, options.merge(disabled: true, class: 'form-control', style: 'display: none;'))
  ...
  def full_name_block(name1, name2, label_text, options = {})
    markup(:div, class: 'form-group py-2') do |m|
      ...
      m.div(class: 'form-row') do
        m.div(class: 'col') do
          m << text_field(name1, options.merge(class: 'form-control', placeholder: 'Family name'))
          m << error_messages_for(name1)
        end
        m.div(class: 'col') do
          m << text_field(name2, options.merge(class: 'form-control', placeholder: 'Given name'))
  ...
  def change_password_button(markup)
    markup.button('変更する',
                  type: 'button',
  ...
  def not_change_password_button(markup)
    markup.button('変更しない',
                  type: 'button',
